...
int main(void) {
  int i=0;
    ...
    scanhome("/mnt/c/Ubuntu/");//listing della home,qui bisogna inserire il path della propria home,il mio è questo dato che ho installata la shell linux su windows 10
...
void *workerthread(void *cs){
        ...
        strcpy(buff,"Ïnserisci Username:\n");
        write(sock,(void *)buff,strlen(buff));

        read(sock,(void *)utente,N);
        sprintf(buff,"Welcome %s\nLista Comandi:\n1.listhome\n2.sd\n",utente);
        write(sock,(void *)buff,strlen(buff));

        len=read(sock,(void *)buff,100);
        if(len>=9 && strncmp(buff,"listhome ",9)){
                pthread_mutex_lock(&mutexlista);
                printf("dasdasd\n");
                invialista(list, sock,"mnt/c/Ubuntu");
